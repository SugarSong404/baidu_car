...
# 导入自定义log模块
from tools_v.log_info import logger
from Baidu_code import MotorConvert, Motors, WheelWrap
from tools_v import PID
...
class CarBase():
    ...
    def set_velocity(self, linear_vx, linear_vy, angular_v):
        if not self.key_error:raise KeyError("主动终止进程")
        # 根据速度计算四个轮子速度
    ...
    def odomery_update(self):
        ...
        while True:
            ...
            # 获取每个轮子的位移
            if linear_wheel_now.size == 0 or linear_wheel_last.size == 0:continue
